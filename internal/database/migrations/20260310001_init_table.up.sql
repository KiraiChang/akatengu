...
CREATE TABLE IF NOT EXISTS snapshots (
                           ...
                           state           TEXT    NOT NULL,  -- JSON，aggregate 當下完整狀態
...
SELECT
    ...
    CASE
        ...
        ELSE COALESCE(SUM(l.remaining_qty * l.unit_cost_twd), 0)
            / SUM(l.remaining_qty)
        END                                                         AS avg_cost_twd,
    COALESCE(SUM(l.remaining_qty * l.unit_cost_twd), 0)         AS total_cost_twd
FROM investments i
         LEFT JOIN investment_lots l ON i.investment_id = l.investment_id
    AND l.status != 'CLOSED'
WHERE i.is_active = 1
...
SELECT
    ...
    SUM(m.realized_gain_twd)                AS total_realized_gain,
    SUM(m.fee_twd + m.tax_twd)             AS total_cost,
    COUNT(*)                                AS sell_count
...
SELECT
    ...
    l.unit_cost,
    l.unit_cost_twd,
    l.remaining_qty * l.unit_cost_twd      AS remaining_cost_twd
