...
func (e eventPeriodAnnualClosedProjector) Project(ctx context.Context, ct *pipelines.Context[state.PeriodAnnualClosedState, payload.PeriodAnnualClosedPayload]) error {
	...
	s.ClosedTxn, err = appendClosingEntry(ctx, year, netIncome, is)
	if err != nil {
		return fmt.Errorf("closing entry: %w", err)
	}

	// 5. 期初結轉：3200 → 3100
	s.OpenedTxn, err = appendOpeningEntry(ctx, year, netIncome)
	if err != nil {
...
func appendOpeningEntry(ctx context.Context, year int, netIncome decimal.Decimal) (payload.TransactionCreatedPayload, error) {
	// 3200 結轉到 3100
