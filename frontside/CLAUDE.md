...
0. **評估需求規模，決定是否分階段實作**
   接獲需求後，**先評估其規模**，判斷是否為大型需求（符合以下任一條件即為大型）：

   - 預計修改檔案超過 **5 個**
   - 預計新增或修改程式碼超過 **200 行**
   - 涉及多個功能模組或跨多個 `src/` 子目錄
   - 需要多輪探索（閱讀大量現有程式碼）才能釐清範圍

   **若判定為大型需求，必須在步驟 1 之前先向使用者說明：**
   1. 評估結果（為何判定為大型需求）
   2. 建議的分階段方案（Phase 1 做什麼、Phase 2 做什麼……）
   3. 本次對話（window）預計執行第幾階段

   **待使用者確認階段劃分後，才繼續執行步驟 1，且本次對話只實作當前階段的內容。**

   > **原因**：對話 window content 上限約為 1M token。大型需求若一次全部實作，中段上下文可能遭截斷，導致前後邏輯不一致或遺漏步驟。分階段可確保每個 window 的負荷在安全範圍內，也讓使用者更容易驗收。
